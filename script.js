...
(function(){
  ...
  // ✅ 급수 변경 즉시 반영
  document.querySelector('#prLevelSelect').addEventListener('change', function(e){ state.prLevel=e.target.value; });
  ...
  function startDrill(){
    ...
    if(items.length===0){ alert('선택한 급수의 문제가 없습니다.'); return; }
    // ✅ 실전쓰기: data.json 순서 그대로 (shuffle 없음)
  ...
  function init(){
    ...
    // auth가 초기화된 경우, 로그인 상태 변화 시 목록 갱신
    // (compat 초기화는 index.html에서 진행)
    if(window.firebase && firebase.auth){ firebase.auth().onAuthStateChanged(function(){ renderRecords(); }); }
  }
