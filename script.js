...
(function(){
  ...
  window.appLog = (function(){
    ...
    function ensure(){
      ...
      badge.addEventListener('click', function(){ box.style.display = box.style.display==='none' ? 'block':'none'; });
    }
    function setBadge(text){ ensure(); badge.textContent=text; }
    function log(){
      ensure();
      var msg=[].slice.call(arguments).map(function(v){ try { return typeof v==='string'? v : JSON.stringify(v); } catch(_) { return String(v); } }).join(' ');
  ...
  function startSession(){ var items=(state.data&&state.data[state.lang]&&state.data[state.lang][state.level]||[]).slice(); if(items.length===0){ alert('선택한 급수의 문제가 없습니다.'); return;} shuffle(items); var qs=items.slice(0,Math.min(state.count,items.length)); state.session={items:qs, idx:0, answers:[], childId:activeChildId(), lang:state.lang, level:state.level, mode:MODE_STANDARD}; $('#progNow').textContent='1'; $('#progTotal').textContent=String(qs.length); $('#qNum').textContent='1'; $('#answerInput').value=''; setFeedback('', ''); show('play'); }
  $('#replayBtn').addEventListener('click', function(){ var s=state.session; if(!s) return; var q=s.items[s.idx]; speak(q, state.rate, s.lang==='ko'?'ko-KR':'en-US'); });
  $('#checkBtn').addEventListener('click', function(){ var s=state.session; if(!s) return; var q=s.items[s.idx]; var a=$('#answerInput').value.trim(); var ok=normalize(a)===normalize(q); s.answers[s.idx]={q:q,a:a,ok:ok}; setFeedback(ok? '정답! 잘했어요 👍':'오답이에요 😅\n정답: '+q, ok?'ok':'no'); });
  $('#nextBtn').addEventListener('click', function(){ var s=state.session; if(!s) return; if(!s.answers[s.idx]){ var q=s.items[s.idx]; var a=$('#answerInput').value.trim(); s.answers[s.idx]={q:q,a:a,ok:normalize(a)===normalize(q)}; } s.idx++; if(s.idx>=s.items.length){ finishSession(); return; } $('#progNow').textContent=String(s.idx+1); $('#qNum').textContent=String(s.idx+1); $('#answerInput').value=''; setFeedback('', ''); $('#answerInput').focus(); });
  ...
  // ====== 실전쓰기 버튼 동작 (채점/저장) ======
  function gradeDrill(autoFill){ var s=state.session; if(!s || s.mode!==MODE_DRILL){ alert('실전쓰기 세션이 없습니다.'); return null; } var total=s.items.length, correct=0, unmarked=0; for(var i=0;i<total;i++){ var cur=s.answers[i]; if(!cur){ if(autoFill){ s.answers[i]={q:s.items[i], a:null, ok:false}; } else { unmarked++; continue; } } if(s.answers[i] && s.answers[i].ok) correct++; } return {total:total, correct:correct, unmarked:unmarked}; }
  var _wiredDrill=false; function wireDrillButtons(){ if(_wiredDrill) return; var gradeBtn=$('#drillGradeBtn'); var saveBtn=$('#drillSaveBtn'); if(!gradeBtn || !saveBtn) return; _wiredDrill=true; gradeBtn.addEventListener('click', function(){ var res=gradeDrill(false); if(!res) return; alert('실전쓰기 채점: '+res.correct+' / '+res.total + (res.unmarked?(' (미표시 '+res.unmarked+'개)'):'')); }); saveBtn.addEventListener('click', function(){ var res=gradeDrill(true); if(!res) return; finishSession(); }); }
  document.addEventListener('DOMContentLoaded', wireDrillButtons);
  (function observePracticeView(){ var el=$('#view-practice'); if(!('IntersectionObserver' in window) || !el){ wireDrillButtons(); return; } var io=new IntersectionObserver(function(ents){ if(ents.some(function(e){return e.isIntersecting;})) wireDrillButtons(); }); io.observe(el); })();
  ...
  if(window.FIREBASE_ENABLED && window.firebase){
    ...
    auth.onAuthStateChanged(function(user){
      ...
      window.appLog.setBadge(uid ? ('클라우드: 로그인됨') : '클라우드: 로그인 안 됨');
      window.appLog.log('[Auth]', uid ? ('로그인 uid='+uid) : '로그아웃 상태');
    });

    auth.signInAnonymously()
      .then(function(cred){ window.appLog.log('[Auth] 익명 로그인 성공 uid=', cred.user && cred.user.uid); })
      .catch(function(e){ window.appLog.log('[Auth] 익명 로그인 실패', e && e.message); });
  } else {
    window.appLog.setBadge('클라우드: 비활성화');
    window.appLog.log('[Firebase] 비활성화 또는 SDK 미로딩');
  }
  ...
  function finishSession(){
    ...
    var entry={ ts:Date.now(), date:new Date().toISOString(), lang:s.lang, level:s.level, total:total, correct:correct, mode:s.mode||MODE_STANDARD, details:s.answers, childId:cid };
    ...
    if(db && uid){
      db.collection('records')
        ...
        .then(function(){ window.appLog.log('[Firestore] 저장 성공', {score: entry.correct+'/'+entry.total, mode: entry.mode}); })
        .catch(function(e){ window.appLog.log('[Firestore] 저장 실패', e && e.message); });
    } else {
      window.appLog.log('[Firestore] 저장 생략 (db/uid 없음)', { hasDb: !!db, uid: uid });
    }
